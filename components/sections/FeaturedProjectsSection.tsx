...
/* Shared card shell — fixed aspect ratio, same size for every card */
function CardShell({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  return (
    <div
      className={`relative rounded-sm border border-white/10 overflow-hidden flex flex-col ${className}`}
  ...
  return (
    <section className="bg-brand-black text-white min-h-[100dvh] flex flex-col justify-center relative overflow-hidden" id="featured">
      ...
      <div className="relative z-10 w-full px-5 md:px-10 lg:px-16 xl:px-24 2xl:px-36 py-12 md:py-16 lg:py-20 xl:py-24">
        ...
        {/* Header */}
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-10 xl:mb-14">
          <div>
          ...
          <p className="text-white/40 font-light text-xs xl:text-sm max-w-xs text-right hidden md:block">
            Each project is built on trust, quality craftsmanship, and a commitment to the communities we serve.
          </p>
        </div>

        {/* Cards grid — equal columns, equal height via aspect-ratio */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 xl:gap-6">
          {projects.map((project, i) => (
            project.comingSoon ? (

              /* ── Coming Soon ── */
              <CardShell key={project.id} className="group">
                {/* Full image, no crop */}
                <img
                  src={project.image}
                  alt="Coming Soon"
                  className="absolute inset-0 w-full h-full object-contain transition-transform duration-700 group-hover:scale-[1.03] bg-brand-black"
                />
                {/* Bottom gradient so badge readable */}
              ...
              >
                <CardShell className="group cursor-pointer">
                  {/* Full image, no crop */}
                  <img
                    src={project.image}
                    alt={project.name}
