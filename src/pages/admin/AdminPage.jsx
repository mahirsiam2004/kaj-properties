...
const IMGBB_API_KEY = import.meta.env.VITE_IMGBB_API_KEY || '837352934eb053d32ea0e4d9ac7da4ba';
  ...
  const handleSubmitUpdate = async (e) => {
    ...
    try {
      ...
      const saveRes = await fetch(url, {
        method,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ title, description: caption, imageUrl, date: date || undefined }),
      });
  ...
  const handleChangePassword = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        let data;
        try { data = JSON.parse(text); } catch { data = null; }
        setSettingsMessage(data?.error || `Server error (${res.status})`);
        return;
      }
      const text = await res.text();
      const data = JSON.parse(text);
      ...
      setSettingsMessage('Cannot connect to server.');
    } finally {
                    ...
                    <div key={update._id} className="ap-card">
                      ...
                      <div className="ap-card-body">
                        ...
                        <div className="ap-card-footer">
                          <span>{update.date ? new Date(update.date).toLocaleDateString() : new Date(update.createdAt).toLocaleDateString()}</span>
